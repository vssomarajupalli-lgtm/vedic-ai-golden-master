...
There is a direct and fatal conflict between the newly proposed `docs/governance/ASTROLOGICAL_PREDICTION_GOVERNANCE_v1.md` (which isolates Mandali) and the actual source code currently running in `registry_data.py` and `evaluator.py` (which tightly couples Mandali).
