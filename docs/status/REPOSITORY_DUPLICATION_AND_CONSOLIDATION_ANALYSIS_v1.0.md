...
* **Architecture documents**: 
  * Overlap observed between `docs/architecture/PHASE14G_ARCHITECTURE_REVIEW.md` and current `KNOWLEDGE_PACKAGE` documents. 
...
* **Knowledge Packages**: 
  * Highly distinct. Minor overlap in definitions across `docs/knowledge/YOGA_INTELLIGENCE_KNOWLEDGE_PACKAGE.md` and `docs/knowledge/DOSHA_INTELLIGENCE_KNOWLEDGE_PACKAGE.md`.
  * *Classification*: Preserve Separately (maintains strict domain boundaries).
* **Formula documents**: 
  * `docs/status/FORMULA_CATEGORY_CATALOG_v1.md` and `docs/status/FORMULA_FAMILY_CATALOG_v1.md` contain overlapping indexing.
...
* **Calibration documentation**: Isolated to `docs/knowledge/CALIBRATION_FRAMEWORK_KNOWLEDGE_PACKAGE.md`. Unique.
* **Engine documentation**: Redundancy exists between `docs/status/ENGINE_OWNERSHIP_AND_DATA_CONTRACT_REGISTRY_v1.0.md` and `docs/status/DEPENDENCY_AUDIT_REPORT.md`. *Classification*: Merge Candidate (consolidate dependency matrices).
...
* **Planet formulas**: Contained in `docs/knowledge/PLANET_INTELLIGENCE_KNOWLEDGE_PACKAGE.md`. *Status*: Unique.
* **House formulas**: Contained in `docs/knowledge/HOUSE_INTELLIGENCE_KNOWLEDGE_PACKAGE.md`. *Status*: Unique.
* **Yoga formulas**: Spread across `docs/knowledge/YOGA_INTELLIGENCE_KNOWLEDGE_PACKAGE.md` and `docs/architecture/FORMULA_REPOSITORY_DATA_MODEL_v1.md`. *Status*: Shared (Requires future review to consolidate mathematical source of truth).
* **Dosha formulas**: Contained in `docs/knowledge/DOSHA_INTELLIGENCE_KNOWLEDGE_PACKAGE.md`. *Status*: Unique.
* **Varga formulas**: Contained in `docs/knowledge/VARGA_INTELLIGENCE_KNOWLEDGE_PACKAGE.md`. *Status*: Unique.
* **Dasha formulas**: Contained in `docs/knowledge/DASHA_INTELLIGENCE_KNOWLEDGE_PACKAGE.md`. *Status*: Unique.
* **Probability formulas**: Overlap between `docs/knowledge/MASTER_PROBABILITY_KNOWLEDGE_PACKAGE.md` and `docs/knowledge/QUESTION_ENGINE_KNOWLEDGE_PACKAGE.md`. *Status*: Duplicated (Question Engine currently hardcodes a 60/40 split). Requires future review for consolidation.
* **Question formulas**: Definitions scattered across `PHASE11` reports and `docs/status/QUESTION_REGISTRY_MASTER_v1.md`. *Status*: Duplicated. Requires future review.
* **Calibration constants**: Unique to `docs/knowledge/CALIBRATION_FRAMEWORK_KNOWLEDGE_PACKAGE.md`. *Status*: Unique.
...
* **Architecture principles**: Found in `docs/governance/ASTROLOGICAL_PREDICTION_GOVERNANCE_v1.md` and `GOLDEN_MASTER_MANIFEST.md`. 
  * *Target State*: Single Canonical Source.
* **Governance rules**: Found in `docs/governance/GOVERNANCE_COMPLIANCE_CHECKLIST_v1.0.md` and multiple `PHASE` reports.
...
* **Merge candidate**: `docs/status/FORMULA_CATEGORY_CATALOG_v1.md` and `docs/status/FORMULA_FAMILY_CATALOG_v1.md`.
