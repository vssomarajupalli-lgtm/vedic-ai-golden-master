...
* **Architecture & Governance documents**: Form the bedrock. Every Knowledge Package and engine implementation holds a strict *Governance Dependency* on the `ENGINE_OWNERSHIP_AND_DATA_CONTRACT_REGISTRY_v1.0.md` and `GOVERNANCE_COMPLIANCE_CHECKLIST_v1.0.md`.
...
* **Formula & Engine documentation**: Hold a *Formula Dependency* on the `FORMULA_CATEGORY_CATALOG_v1.md`.
...
* **Core architecture documents**: e.g., `ASTROLOGICAL_PREDICTION_GOVERNANCE_v1.md`. Loss breaks overarching systemic rules.
* **Knowledge Packages**: e.g., `PLANET_INTELLIGENCE_KNOWLEDGE_PACKAGE.md`. Loss turns deterministic math into undocumented legacy code.
* **Canonical formula definitions**: Loss prevents any future auditing or correction of astrological mathematics.
* **Engine governance**: e.g., `ENGINE_OWNERSHIP_AND_DATA_CONTRACT_REGISTRY_v1.0.md`. Loss re-introduces ambiguous ownership and overlapping authority.
* **Decision Registers**: e.g., `REPOSITORY_REFINEMENT_DECISION_REGISTER_v1.0.md`. Loss destroys the audit trail of why the repository looks the way it does.
