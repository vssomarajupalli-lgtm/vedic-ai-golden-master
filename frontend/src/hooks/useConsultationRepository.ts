...
// Helper: Get canonical birth data hash
function getBirthDataHash(): string {
  const { canonicalContent, machineIndex } = useChartStore.getState();
  const birthData = {
    date: canonicalContent?.native_info?.dob || '',
...
async function verifySnapshotIntegrity(snapshot: any): Promise<boolean> {
  ...
  const birthMatch = snapshot.metadata.birthDataHash === getBirthDataHash();
  const formulaMatch = snapshot.formulaRegistryHash === getFormulaRegistryInfo().hash;
  const calibrationMatch = snapshot.calibrationProfileHash === getCalibrationProfileInfo().hash;
  const gitMatch = snapshot.git.commit === getGitInfo().commit;
